...
<head>
    ...
    <title>Kepler Mobile-Ready</title>
    <style>
        * { box-sizing: border-box; }
        ...
        canvas { 
            ...
            touch-action: none; /* Critical: stops browser from scrolling while dragging */
        }
        #controls { 
...
<div id="controls">
    <h3>Kepler Recon <small style="font-weight:normal; color:#666;">(Drag colored circles)</small></h3>
    <div id="ui"></div>
</div>
...
function drawCircle(x, y, r, col, lw) {
    ...
    ctx.strokeStyle = '#000'; ctx.lineWidth = lw; ctx.stroke();
}
...
function render() {
    ...
    [orbit_m, orbit_e].forEach(orb => {
        ctx.beginPath();
        for(let i=0; i<orb[0].length; i++) ctx.lineTo(cx + orb[0][i]*scale, cy - orb[1][i]*scale);
        ctx.stroke();
    });
    ...
    marsPos.forEach((p, i) => {
        ...
        th_s[i].forEach((ts, j) => {
            ...
            ctx.setLineDash([]); drawCircle(cx + ex*scale, cy - ey*scale, 5, color, 1);
        });
        ...
        ctx.moveTo(rx-10, ry); ctx.lineTo(rx+10, ry); ctx.moveTo(rx, ry-10); ctx.lineTo(rx, ry+10); ctx.stroke();
